...
class TestDecomposeClaims:
    ...
    async def test_successful_decomposition(self):
        ...
        claims_json = json.dumps(["Paris is the capital of France", "The Eiffel Tower is in Paris"])
        mock_provider.complete.return_value = CompletionResponse(
            content=claims_json, model="test-model", finish_reason="stop"
        )
    ...
    async def test_invalid_json_fallback(self):
        ...
        mock_provider.complete.return_value = CompletionResponse(
            content="This is not valid JSON", model="test-model", finish_reason="stop"
        )

        original_text = "Some response text"
        result = await decompose_claims(llm_response=original_text, provider=mock_provider)
    ...
    async def test_filters_short_claims(self):
        ...
        claims_json = json.dumps(
            ["This is a valid claim", "x", "", "Another valid claim"]  # Too short  # Empty
        )
        mock_provider.complete.return_value = CompletionResponse(
            content=claims_json, model="test-model", finish_reason="stop"
        )

        result = await decompose_claims(llm_response="test", provider=mock_provider)
...
class TestDecomposerNode:
    ...
    async def test_node_execution(self):
        ...
        claims_json = json.dumps(["Claim 1", "Claim 2"])
        mock_provider.complete.return_value = CompletionResponse(
            content=claims_json, model="test-model", finish_reason="stop"
        )
