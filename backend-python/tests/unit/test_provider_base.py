...
class TestCompletionRequest:
    ...
    def test_request_creation(self):
        ...
        request = CompletionRequest(
            messages=messages, model="llama3.2", temperature=0.7, max_tokens=100
        )
    ...
    def test_create_messages(self):
        ...
        class MockProvider(LLMProvider):
            async def complete(self, request):
                return CompletionResponse(content="test", model=self.model, finish_reason="stop")

            async def health_check(self):
                return True

        provider = MockProvider(model="test-model")
        messages = provider.create_messages(system_prompt="You are helpful", user_message="Hello")
